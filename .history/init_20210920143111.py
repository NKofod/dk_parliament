...
def fetch_periods(): 
    ...
    while base_url:
        ...
        for entry in periodData:
            outDict[entry['id']] = {
                'startdate': entry['startdato'],
                'enddate': entry['slutdato'],
                'type': entry['type']
            }
        try: 
        ...
        for group in group_data:
            try: 
                tmp_dict = {
                    "id": group["id"],
                    "name": group["navn"],
                    "short_name": group["gruppenavnkort"],
                    "start": group["startdato"],
                    "end": group["slutdato"],
                    "periodid": group["periodeid"],
                }
            except KeyError:
                try: 
                    tmp_dict = {
            ...
            try: 
                out_dict[group["gruppenavnkort"]].append(tmp_dict)
            except KeyError:
                out_dict[group["gruppenavnkort"]] = []
                out_dict[group["gruppenavnkort"]].append(tmp_dict)
        try: 
